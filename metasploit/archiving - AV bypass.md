placing a password on the archive bypasses a lot of common anti-virus signatures today. However, the downside of this process is that they will be raised as notifications in the AV alarm dashboard as being unable to be scanned due to being locked with a password

checking AV detection with virus total CLI
```
...
example result test from virustotal
```
jodijotar@root$ msf-virustotal -k <API key> -f test2
...
RAR utility - https://www.rarlab.com/download.htm
	works precisely like WinRAR