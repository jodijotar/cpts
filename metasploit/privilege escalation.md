...
when exploiting web RCE vulnerabilities often we land on a low privillege user, so use this recon to help us identify other vulnerabilities:

recon module to privilege escalation
```
msf6 > use post/multi/recon/local_exploit_suggester
```

for the suggested payload becomes more precise and reliable, its good to check the system architecture:
```
meterpreter > sysinfo
```
---
## impersonating other process

meterpreter process migration example:
```
meterpreter > getuid
